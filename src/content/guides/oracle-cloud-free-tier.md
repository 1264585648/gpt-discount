...
warning: "注册时必须选择 Home Region，之后通常不能自行更改；Always Free 计算和数据库主要只能在 Home Region 创建。Oracle 当前官方文档中的 Ampere A1 免费计算配额为每月 1,500 OCPU 小时和 9,000 GB 内存小时，对纯 Always Free 租户相当于总计 2 OCPU、12 GB 内存。试用到期后，超过 Always Free 限额的资源会被回收；只有主动升级为 Pay As You Go 后，超额资源才会产生账单。"
...
steps:
  - title: "确认新用户资格和支付方式"
    description: "Oracle Cloud Free Tier 原则上每人只允许一个免费账号。Oracle 当前接受信用卡，以及能够按信用卡方式交易的部分借记卡；注册时可能产生临时预授权，通常由银行在 3–5 天内解除。"
    note: "官方资格和支付说明：[[OCI Cloud Free Tier FAQ|https://www.oracle.com/cloud/free/faq/]]"
  - title: "选择 Home Region"
    description: "注册页面会要求选择 Home Region。Always Free 计算实例、块存储和 Autonomous Database 主要需要在该区域创建，而 Home Region 之后通常不能自行更换。优先选择离主要用户较近、服务完整且当前有免费实例容量的区域。"
    note: "如果所选区域经常出现 Out of host capacity，免费实例可能暂时无法创建，账号也不能因此直接迁移 Home Region。"
  - title: "从官方 Free Tier 页面注册"
  ...
  - title: "填写真实的账号和账单资料"
    description: "使用本人长期邮箱，填写姓名、国家或地区、地址和手机号。账单姓名、地址和银行卡信息应尽量一致；缩写、随意英文名或与银行记录不符的信息容易在支付验证阶段失败。"
  - title: "完成手机号和银行卡验证"
    description: "按页面接收验证码并提交银行卡。Oracle 使用银行卡验证身份，可能出现小额临时授权，但官方说明在主动升级前不会因为普通 Free Tier 用量产生实际扣款。验证连续失败时，先确认发卡行允许国际线上交易。"
  - title: "首次登录后确认赠金与到期日"
  ...
  - title: "启用 MFA 并保护管理员账号"
    description: "为主账号启用多因素认证，不要把密码、API Key、私钥或配置文件上传到公开仓库。日常项目尽量创建单独用户、组和最小权限策略。"
  - title: "创建资源时选择 Always Free Eligible"
    description: "控制台会对符合长期免费条件的资源显示 Always Free Eligible。创建虚拟机、数据库、负载均衡器和存储前确认该标签，并再次检查区域、规格和数量。"
  ...
  - title: "按当前配额创建 AMD 或 Arm 虚拟机"
    description: "AMD 路线最多可使用 2 台 VM.Standard.E2.1.Micro。Arm 路线使用 VM.Standard.A1.Flex，当前官方文档对纯 Always Free 租户给出的总额度相当于 2 OCPU 和 12 GB 内存，可集中在 1 台，或拆分为最多 2 台。"
    note: "若出现 Out of host capacity，可尝试同一区域内其他可用域，或稍后再创建。"
  - title: "把 200 GB 块存储作为总池计算"
    description: "Always Free 提供总计 200 GB 的启动盘和块存储，以及最多 5 个卷备份。每台计算实例默认会占用启动盘，因此实例数量、启动盘和附加磁盘会共同消耗这 200 GB。删除虚拟机时还要确认是否同时删除启动盘。"
  - title: "需要数据库时选择免费 Autonomous Database"
    description: "账号最多可创建 2 个 Always Free Autonomous AI Database，每个当前为 1 OCPU、20 GB 存储。数据库也受 Home Region 和区域支持限制，创建页面没有 Always Free 标签时不要继续。"
  - title: "设置预算与费用通知"
    description: "在 Billing & Cost Management 中创建 Budget 和通知规则。预算通知不等于自动停止资源；仍需定期查看 Cost Analysis，并在试用结束前删除不再需要的付费规格。"
  - title: "试用结束前区分免费和付费资源"
    description: "明确标记 Always Free、并且没有超过数量和规格上限的资源可以保留。试用赠金创建的付费实例、额外存储、付费数据库或其他非免费服务，应在到期前导出数据并删除。"
  - title: "检查 Ampere A1 是否超过长期免费上限"
    description: "试用期间可能利用赠金创建超过长期免费上限的 A1 实例。试用结束时若 A1 总量高于纯 Always Free 租户允许的 2 OCPU、12 GB，相关实例可能被停用，并在 30 天后删除。到期前应主动缩减到免费范围。"
  - title: "避免长期空闲导致资源被回收"
    description: "Oracle 可能回收空闲 Always Free 计算实例。需要长期保留时，应定期登录、更新系统并实际使用资源，同时保持独立备份。"
  - title: "确定需要时再升级 Pay As You Go"
    description: "试用到期后不升级，Always Free 资源仍可继续使用，也不会自动按量扣费。升级后任何超出免费额度的资源都会正常计费，而且没有直接降回纯免费账号的选项。"
faq:
  - question: "Oracle Cloud Free Tier 会自动扣费吗？"
    answer: "不会。银行卡用于身份验证；只有主动升级为 Pay As You Go 后，超出 Always Free 配额的资源才会开始计费。未升级时，试用赠金到期后的付费资源会被回收。"
  ...
  - question: "现在还是 4 OCPU、24 GB 免费吗？"
    answer: "不是。Oracle 当前 Always Free 文档显示，A1 每月免费额度为 1,500 OCPU 小时和 9,000 GB 内存小时，对纯 Always Free 租户相当于总计 2 OCPU、12 GB 内存。"
...
旧教程普遍写 4 OCPU、24 GB，但当前 Oracle 官方文档对纯 Always Free 租户给出的等效上限是 **2 OCPU、12 GB**。

### 试用资源与 Always Free 资源混在一起
